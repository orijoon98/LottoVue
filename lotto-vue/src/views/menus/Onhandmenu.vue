  ...
  <div>
    <div class="inputlotto">
      <b-input-group prepend="번호" class="mb-2">
        <b-form-input
          ...
          @focus="
            ...
            $event.target.select();
          "
          @blur="$event.target.placeholder = '회차'"
          oninput="javascript: if (this.value.length >= 4) {this.value = this.value.slice(0, 4); document.getElementById('drwtNo1').focus();}"
        ...
        <b-form-input
          ...
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('drwtNo2').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
        <b-form-input
          type="number"
          ...
          id="drwtNo2"
          @focus="$event.target.select()"
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('drwtNo3').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
        <b-form-input
          type="number"
          ...
          id="drwtNo3"
          @focus="$event.target.select()"
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('drwtNo4').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
        <b-form-input
          type="number"
          ...
          id="drwtNo4"
          @focus="$event.target.select()"
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('drwtNo5').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
        <b-form-input
          type="number"
          ...
          id="drwtNo5"
          @focus="$event.target.select()"
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('drwtNo6').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
        <b-form-input
          type="number"
          ...
          id="drwtNo6"
          @focus="$event.target.select()"
          oninput="javascript: if (this.value.length >= 2) {this.value = this.value.slice(0, 2); document.getElementById('inputbtn').focus();}"
          @keyup.enter="getInput"
        ></b-form-input>
      ...
      <div class="alertContainer">
        ...
        <b-button id="error3" v-b-modal.err3 ref="showError3"
          >유효기간오류</b-button
        >
        <b-modal
          title="오류"
        ...
        <b-button id="error4" v-b-modal.err4 ref="showError4"
          >숫자범위오류</b-button
        >
        <b-modal
          title="오류"
        ...
        <b-button id="error5" v-b-modal.err5 ref="showError5"
          >숫자중복오류</b-button
        >
        <b-modal
          title="오류"
...
export default {
  ...
  methods: {
    checkInput() {
      ...
      for (var n of allNum) {
        // 숫자 7개를 모두 입력하지 않으면 경고
        if (n == "") {
          this.$refs.showError1.click();
          return false;
        }
      }
      if (parseInt(this.number) > parseInt(this.drwNo[52])) {
        // 업데이트 안된 회차면 경고
        this.$refs.showError2.click();
        return false;
      }
      if (parseInt(this.number) < parseInt(this.drwNo[0])) {
        // 기한이 지난 회차면 경고
    ...
    showResult() {
      ...
      var cnt = 0,
        bnusCnt = 0;
      var winNum = [
        this.drwtNo1[targetIndex],
        this.drwtNo2[targetIndex],
        this.drwtNo3[targetIndex],
        this.drwtNo4[targetIndex],
        this.drwtNo5[targetIndex],
        this.drwtNo6[targetIndex],
        this.bnusNo[targetIndex],
      ];
      var inputArr = [
        this.one,
        this.two,
